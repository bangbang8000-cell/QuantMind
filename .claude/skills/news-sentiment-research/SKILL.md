...
报告标准结构（12 章）：数据基础 → 事件研究 → 来源特征 → 时间特征 → 信号强度 → 价格行为 → 事件标签 → 策略回测 → 极端案例 → 规律清单 → 风险局限 → 结论。
...
### P0：消除到期亏损（-104,346 元，最大亏损源）

25 笔 max_hold 出场贡献全部净亏损。两条路：
1. **缩短持仓**：MAX_HOLD_DAYS 20 → 10（情绪效应 T+10 后转弱，事件研究已证明）
2. **到期前均线离场**：持仓 ≥10 天且跌破 5 日均线 → 提前离场
...
- [[stock-skill-dual-copy-sync]] — 改项目级 skill 必须 cp 同步到 ~/.claude/skills/
