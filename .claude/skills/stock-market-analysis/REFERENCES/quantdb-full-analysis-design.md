...
- **⚠️ l2 停更**：分区止于 dt=20260227。若目标日期之后 l2 型字段全 NaN，报告必须写「l2 资金流数据缺失（厂商停更），资金面结论降级为不可验证」
...
| 标准分析 | 3-5 min | 日常投研：全部 7 层 + 财务 4 期 + L6 默认+融合 |
| 深度尽调 | 10+ min | 用户明确要"全方位"：全部 7 层 + L6 全模型逐拉 + 财务三表 8 期 + 5 年估值分位 + 融资融券 30 日 + 股东户数趋势 + 分红历史 + 情景推演 |
