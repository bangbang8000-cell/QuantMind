...
| 方式 | 适用场景 | 入口 |
| --- | --- | --- |
| 完整离线部署 | 迁移完整业务数据、模型与 Qlib 数据；网络不稳定 | `offline-deploy.sh` |
| 在线源码部署 | 新服务器可稳定访问代码和镜像仓库 | `deploy.sh` |
...
离线包 CDN 目录默认是 `https://cdn.quantmind.cloud/quantmind-offline`，应包含：
