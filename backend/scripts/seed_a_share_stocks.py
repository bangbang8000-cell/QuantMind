...
_BUILTIN_SENTIMENT_POS = [
    ...
    # 2026-08 扩充（增强字典法区分度，FinBERT 不可用时兜底也有方向）
    ("涨停", 1.0), ("封板", 0.9), ("一字板", 0.9), ("业绩预增", 0.9),
    ("预增", 0.8), ("同比大增", 0.85), ("高增长", 0.8), ("扭亏为盈", 0.9),
    ("翻倍", 0.85), ("历史新高", 0.9), ("创历史新高", 0.95), ("涨停板", 0.95),
    ("获大单", 0.8), ("签大单", 0.8), ("战略合作", 0.6), ("重大合同", 0.7),
    ("回购注销", 0.75), ("大额回购", 0.7), ("高送转", 0.7), ("派息", 0.6),
    ("业绩超预期", 0.9), ("超市场预期", 0.8), ("景气", 0.6), ("回暖迹象", 0.5),
    ("需求旺盛", 0.7), ("量价齐升", 0.75), ("毛利率提升", 0.65), ("降本增效", 0.6),
    ("供不应求", 0.7), ("涨价", 0.6), ("提价", 0.6), ("景气度上行", 0.7),
    ("增持计划", 0.75), ("举牌", 0.7), ("股权激励", 0.55), ("核心技术突破", 0.7),
    ("获批上市", 0.7), ("产品放量", 0.7), ("海外拓展", 0.55), ("新签订单", 0.65),
]
_BUILTIN_SENTIMENT_NEG = [
    ...
    # 2026-08 扩充
    ("闪崩", -0.95), ("崩盘", -0.95), ("退市风险", -0.95), ("重大违法", -0.95),
    ("业绩暴雷", -1.0), ("业绩亏损", -0.85), ("净利润下滑", -0.7), ("同比下滑", -0.7),
    ("大幅下滑", -0.8), ("不及预期", -0.7), ("低于预期", -0.7), ("预亏", -0.9),
    ("由盈转亏", -0.9), ("商誉减值风险", -0.8), ("存货跌价", -0.6), ("坏账", -0.6),
    ("资金链断裂", -0.95), ("债务危机", -0.9), ("债务逾期", -0.85), ("失信", -0.85),
    ("被执行人", -0.8), ("冻结", -0.7), ("查封", -0.8), ("处罚", -0.8),
    ("警示函", -0.75), ("监管函", -0.7), ("问询函", -0.6), ("立案", -0.9),
    ("调查", -0.7), ("被调查", -0.8), ("违规担保", -0.9), ("财务造假", -1.0),
    ("造假", -0.9), ("内幕交易", -0.85), ("操纵市场", -0.85), ("减持计划", -0.75),
    ("拟减持", -0.7), ("清仓减持", -0.8), ("抛售", -0.7), ("破位", -0.7),
    ("阴跌", -0.6), ("回调", -0.4), ("承压", -0.5), ("利空出尽", -0.3),
    ("风险提示", -0.5), ("质押风险", -0.7),
    # 监管/处罚类（FinBERT 对"暂停开户/被重锤"等词不敏感，字典法兜底）
    ("被重锤", -0.8), ("重锤", -0.6), ("暂停开户", -0.9), ("暂停业务", -0.85),
    ("暂停办理", -0.85), ("暂停资格", -0.85), ("被罚", -0.85), ("罚款", -0.8),
    ("行政处罚", -0.85), ("监管措施", -0.6), ("整改", -0.5), ("责令改正", -0.7),
    ("警示", -0.6), ("通报批评", -0.75), ("公开谴责", -0.8), ("暂停展业", -0.9),
]
_BUILTIN_EVENTS = [
    ("回购", "回购"), ("增持", "增持"), ("减持", "减持"),
