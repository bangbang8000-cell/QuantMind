...
def _frame(day: str, close: float) -> pd.DataFrame:
    return pd.DataFrame({
        "symbol": ["600001.SH", "000001.SZ"], "date": [day, day],
        "open": [close - 1, close - 1], "high": [close + 1, close + 1],
        "low": [close - 2, close - 2], "close": [close, close],
        "volume": [100, 100], "amount": [1000, 1000], "l1_alpha": [0.1, 0.2],
    })


def test_reads_single_source_with_logical_field_alias(tmp_path):
    _write_factor_partition(tmp_path, "l1_l2_factors", _frame("2024-01-02", 10), "20240102")
    ...
    data = reader.read_day(
        "l1_l2_factors", features=["alpha"],
        feature_sources={"alpha": "l1_alpha"}, trade_date="2024-01-02",
    )

    assert set(data["symbol"]) == {"600001.SH", "000001.SZ"}
    assert data["alpha"].tolist() == [0.1, 0.2]


def test_rejects_source_without_common_ohlcv(tmp_path):
    _write_factor_partition(
        tmp_path, "l2_factors", pd.DataFrame({"symbol": ["600001.SH"], "date": ["2024-01-02"], "l2_alpha": [1.0]}), "20240102",
    )
...
def test_forward_labels_use_future_close_without_snapshot(tmp_path):
    for day, close in [("2024-01-02", 10), ("2024-01-03", 11), ("2024-01-04", 12)]:
        _write_factor_partition(tmp_path, "l1_factors", _frame(day, close), day.replace("-", ""))
    reader = QuantDBFactorReader(tmp_path)
    frame = reader.read_range("l1_factors", features=["l1_alpha"], start="2024-01-02", end="2024-01-04")
    labels = reader.forward_labels(frame, horizon=1, signal_lag_days=0)
    first = labels[(labels["symbol"] == "600001.SH") & (labels["trade_date"] == pd.Timestamp("2024-01-02"))]
