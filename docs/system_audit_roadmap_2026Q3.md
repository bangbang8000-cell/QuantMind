...
| **H-3** | 2026 parquet 内部 prefix + suffix 双格式同存，31.4 万对重复 key（51%）| groupby 重复加权污染；**最近几次训练用了这个 parquet** |
...
| 任务 | 工作量 | 优先级 |
|------|--------|--------|
| 1.1 统一 2016-2025 vs 2026 的 symbol 格式（全 prefix 或全 suffix，选 prefix） | 1 hr | S-1 |
| 1.2 重新计算 2016-2025 段的 `adj_factor`（用 parquet `factor` 字段，而不是硬写 1） | 2 hr | S-1 / H-2 |
| 1.3 移除 PG 和 parquet 里的指数行（000300.SH 等 5 个） | 30 min | S-4 |
| 1.4 重新计算 2016-2025 段 `volume_ratio_5/_20`（去除 clip，改用 `/` 而非 `-1`）| 1 hr | S-2 |
| 1.5 dedup 2026 parquet 内部重复 key（保留 suffix 删除 prefix） | 30 min | H-3 |
| 1.6 标记 2026-06-22~24 为 incomplete + **重新跑数据同步**（baostock/akshare 补全） | 1 hr | S-3 |
| 1.7 单位规范化：把 `return_*` 全部 ×100 改成百分数与 `pct_change` 一致；或反过来全改小数 | 2 hr | M-1 |
| 1.8 跑数据质量验证脚本，确认所有 S/H 问题消除 | 1 hr | — |
