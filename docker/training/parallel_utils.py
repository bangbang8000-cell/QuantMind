...
并行度控制：
    - 默认 ``min(CPU 核数, 特征数, 交易日数)``
    - 环境变量 ``TRAIN_IC_WORKERS=4`` 可显式指定 worker 数（0/1 = 串行）
...
def compute_daily_ics(
    ...
    Args:
        ...
        n_workers: 显式 worker 数；``None`` 时取
            ``min(CPU 核数, 特征数, 交易日数)``，并允许
            ``TRAIN_IC_WORKERS`` 环境变量覆盖（显式传参时环境变量被忽略）
    """
    ...
    workers = cpu_count if n_workers is None else n_workers
    env_workers = os.getenv("TRAIN_IC_WORKERS")
    if env_workers and env_workers.strip().isdigit() and n_workers is None:
    ...
    workers = min(workers, len(feats), n_dates)
    workers = max(1, workers)
