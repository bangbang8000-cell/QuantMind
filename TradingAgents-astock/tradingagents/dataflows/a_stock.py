...
def _build_name_code_map() -> tuple[dict[str, str], dict[str, str]]:
    """Build name→code and code→name maps (mootdx with HTTP fallback)."""
    ...
    try:
        ...
        logger.warning("mootdx stock list failed (%s), falling back to HTTP", e)

    # Fallback: fetch popular stocks via Tencent quote API
    if not n2c:
        logger.info("Building name-code map via Tencent fallback")
        try:
            import urllib.request
            # Fetch a broad set of stock codes from Tencent
            for prefix, market_codes in [("sz", ["000001","000002","000063","000333","000651","000858",
                                                  "002230","002304","002352","002415","002475","002594",
                                                  "002714","002812","002916","300015","300059","300124",
                                                  "300142","300347","300408","300433","300450","300498",
                                                  "300529","300601","300628","300750","300760","300999"]),
                                         ("sh", ["600000","600009","600016","600028","600030","600036",
                                                  "600048","600050","600089","600104","600111","600196",
                                                  "600276","600309","600406","600436","600519","600585",
                                                  "600690","600745","600809","600887","600900","601012",
                                                  "601088","601166","601211","601288","601318","601398",
                                                  "601601","601668","601688","601766","601818","601857",
                                                  "601888","601899","601919","601985","601988","601989",
                                                  "603259","603288","603501","603799","603986","688981"])]:
                codes_str = ",".join(f"{prefix}{c}" for c in market_codes)
                ...
                for line in raw.strip().split(";"):
                    ...
                    if len(vals) < 5:
                        continue
                    code = key[2:]  # strip sh/sz prefix
                    name = vals[1].strip()
                    if name and _re.match(r"^[036]\d{5}$", code):
                        clean_name = name.replace(" ", "").replace("　", "")
                        n2c[clean_name] = code
                        c2n[code] = clean_name
        except Exception as e2:
            logger.error("Tencent fallback also failed: %s", e2)
...
def _get_mootdx_client():
    ...
    if _mootdx_client is None:
        try:
            ...
            logger.warning("mootdx client init failed: %s", e)
